...
export default function DasMeiPage() {
  ...
  return (
    <Container className="py-10">
      <Breadcrumbs items={[{ href: "/calculadora-das-mei", label: "Calculadora DAS MEI" }]} />
      <h1 className="text-3xl font-black text-slate-950">Calculadora DAS MEI</h1>
      <p className="mt-3 max-w-3xl leading-7 text-slate-600">Veja uma estimativa mensal do DAS conforme o tipo de MEI e a atividade informada.</p>
      <div className="mt-8 grid gap-6 lg:grid-cols-2">
        <section className="rounded-2xl border border-slate-200 bg-white p-5">
          <div className="grid gap-4">
            ...
            <label>Atividade<select value={activity} onChange={(e) => setActivity(e.target.value as ActivityType)}><option value="comercio">Comércio/Indústria</option><option value="servicos">Serviços</option><option value="comercio-servicos">Comércio e Serviços</option></select></label>
          </div>
        </section>
        <section className="grid gap-4">
          <ResultCard title="Valor estimado mensal" value={formatCurrency(value)} tone="green">Valores DAS {meiConfig.dasYear} revisados manualmente no projeto.</ResultCard>
      ...
      <article className="prose-mei mt-10">
        ...
        <p>O DAS é a guia mensal do MEI. A composição pode envolver contribuição ao INSS e tributos como ICMS ou ISS, conforme a atividade exercida.</p>
      </article>
