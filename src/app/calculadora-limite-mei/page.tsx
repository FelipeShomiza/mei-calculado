...
export default function LimiteMeiPage() {
  ...
  return (
    <Container className="py-10">
      <Breadcrumbs items={[{ href: "/calculadora-limite-mei", label: "Calculadora de limite MEI" }]} />
      <h1 className="text-3xl font-black text-slate-950">Calculadora de Limite MEI</h1>
      <p className="mt-3 max-w-3xl leading-7 text-slate-600">Informe o mês de abertura e o faturamento acumulado para estimar o limite proporcional do ano.</p>
      <div className="mt-8 grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
        <section className="rounded-2xl border border-slate-200 bg-white p-5">
          <div className="grid gap-4">
            <label>Ano de referência<input value={year} onChange={(e) => setYear(e.target.value)} inputMode="numeric" /></label>
            ...
            <label>Faturamento atual em {year}<input value={revenue} onChange={(e) => setRevenue(e.target.value)} placeholder="Ex.: 35.000,00" inputMode="decimal" /></label>
            <label>Tipo de MEI<select value={type} onChange={(e) => setType(e.target.value as MeiType)}><option value="comum">MEI comum</option><option value="caminhoneiro">MEI Caminhoneiro / Transportador Autônomo de Cargas</option></select></label>
          </div>
        </section>
        <section className="grid gap-4">
          <AlertBox type={status}>
            {exceeded ? "Você pode ter ultrapassado o limite. Procure um contador ou canal oficial antes de tomar decisões." : status === "success" ? "Você usou menos de 70% do limite estimado." : status === "warning" ? "Atenção: você está entre 70% e 90% do limite." : "Cuidado: você já passou de 90% do limite estimado."}
          </AlertBox>
      ...
      <article className="prose-mei mt-12">
        ...
        <ul>
          <li>Aberto em janeiro: 12 meses ativos.</li>
          <li>Aberto em julho: 6 meses ativos.</li>
          <li>Aberto em novembro: 2 meses ativos.</li>
        </ul>
      </article>
      ...
      <AlertBox type="info"><strong>Aviso:</strong> Este site não é oficial e os cálculos são informativos.</AlertBox>
    </Container>
  );
}
